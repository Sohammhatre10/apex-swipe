...
// Re-using the beautiful styles from the login page!
...
export default function RegisterPage() {
  ...
  return (
    <div className={styles.authContainer}>
      <Link href="/" style={{ position: 'absolute', top: '40px', left: '40px', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-secondary)' }}>
      ...
      >
        <div className={styles.header}>
          ...
          <h1 className={styles.title}>Join ApexSwipe</h1>
          <p className={styles.subtitle}>Set up your profile and get your first AI-curated deck</p>
        </div>

        <form className={styles.form} onSubmit={onSubmit}>
          {error && <div style={{ color: 'var(--accent-red)', fontSize: '0.9rem', textAlign: 'center' }}>{error}</div>}
        ...
        <div className={styles.footer}>
          Already have an account? <Link href="/auth/login" className={styles.footerLink}>Log in</Link>
        </div>
      </motion.div>
    </div>
  );
